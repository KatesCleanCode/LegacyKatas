...
//	test ob alle items verändert werden
	...
	private static GildedRose createAppWithItem(String itemName, int initialSellIn, int initialQuality) {
		Item[] items = new Item[] { new Item(itemName, initialSellIn, initialQuality) };
		return new GildedRose(items);
	}

	private static GildedRose createAppWithSimpleItem(int initialSellIn, int initialQuality) {
		return createAppWithItem("foo", initialSellIn, initialQuality);
	}

	private static GildedRose createAppWithAgedBrieItem(int initialSellIn, int initialQuality) {
		return createAppWithItem("Aged Brie", initialSellIn, initialQuality);
	}

	private static GildedRose createAppWithSulfarasItem(int initialSellIn, int initialQuality) {
		return createAppWithItem("Sulfuras, Hand of Ragnaros", initialSellIn, initialQuality);
	}

	private static GildedRose createAppWithBackstagePassesItem(int initialSellIn, int initialQuality) {
		return createAppWithItem("Backstage passes to a TAFKAL80ETC concert", initialSellIn, initialQuality);
	}

	private static GildedRose createAppWithConjuredItem(int initialSellIn, int initialQuality) {
